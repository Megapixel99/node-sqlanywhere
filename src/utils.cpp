...
/*********************************************************************************/
{
	...
	for (c = 0; c < num_cols; c++)
	{
		...
		for (unsigned int r = 0; r < num_rows; r++)
		{
			...
			{
				param_double[r] = bind_params->Get(context, c).ToLocalChecked()->NumberValue(context).FromJust();
			}
			else if (bind_params->Get(context, c).ToLocalChecked()->IsString())
			{
				String::Utf8Value paramValue(isolate, (bind_params->Get(context, c).ToLocalChecked()->ToString(context)).ToLocalChecked());
...
/*************************************************************************/
{
	...
	for (unsigned int i = 0; i < bind_params->Length(); i++)
	{
		...
		if (bind_params->Get(context, i).ToLocalChecked()->IsInt32())
		{
			...
			*param_int = bind_params->Get(context, i).ToLocalChecked()->Int32Value(context).FromJust();
			ex->addInt(param_int);
		...
		else if (bind_params->Get(context, i).ToLocalChecked()->IsNumber())
		{
			...
			*param_double = bind_params->Get(context, i).ToLocalChecked()->NumberValue(context).FromJust(); // Remove Round off Error
			ex->addNum(param_double);
			param.value.buffer = (char *)(param_double);
			param.value.type = A_DOUBLE;
		}
		...
		{
			String::Utf8Value paramValue(isolate, (bind_params->Get(context, i).ToLocalChecked()->ToString(context)).ToLocalChecked());
			const char *param_string = (*paramValue);
...
/*************************************************************/
{
	...
	{
		Local<String> key = props->Get(context, i).ToLocalChecked().As<String>();
...
/***************************************************************/
{
	...
	if (args.Length() == 1)
	{
		...
		{
			HashToString(isolate, args[0]->ToObject(context).ToLocalChecked(), _arg);
		}
